...
class WeatherClient:
    ...
    def calculate_3h_pressure_trend(self) -> Dict[str, Any]:
        """
        NYTT: Beräkna 3-timmars trycktrend (meteorologisk standard)
        ...
        try:
            ...
            # Hitta mätning ~3 timmar tillbaka
            now = datetime.now()
            ...
            # Hitta närmaste mätning runt 3h tillbaka
            best_match = None
            best_diff = float('inf')
            ...
            if not best_match:
                return {
                    ...
                    'change': 0.0,
                    'period_hours': 0,
                    'reason': 'Ingen matchande mätning'
                }
            ...
            # Klassificera trend (meteorologisk standard: ±2 hPa/3h)
            if time_diff_hours < 1.5:  # Kräv minst 1.5h mellan mätningar
                return {
                    'trend': 'insufficient_data',
                    'change': pressure_change,
                    'period_hours': time_diff_hours,
                    'reason': f'För kort period ({time_diff_hours:.1f}h)'
                }
            ...
            self.logger.info(f"📊 3h trycktrend: {trend} ({pressure_change:+.1f} hPa på {time_diff_hours:.1f}h)")
...
# ============================================================
    ...
    def combine_weather_data(self, smhi_data: Dict, netatmo_data: Dict, sun_data: Dict, observations_data: Dict = None) -> Dict[str, Any]:
        ...
        1. Netatmo Rain Gauge (5 min fördröjning) - HÖGSTA PRIORITET
           → Om Netatmo säger 0mm → regnar INTE (även om SMHI säger annat)
           → Om Netatmo API-fel → fortsätt till steg 2
        2. SMHI Observations (10-60 min fördröjning) - Fallback
        3. SMHI Prognoser - Sista fallback

        Args:
            smhi_data: SMHI väderdata (prognoser, vind, nederbörd, VINDRIKTNING, VINDBYAR)
        ...
        # PRIORITERING: Netatmo för lokala mätningar, NETATMO RAIN GAUGE för nederbörd, OBSERVATIONS för fallback, SMHI för prognoser + VINDRIKTNING + VINDBYAR
        ...
        if smhi_data:
            ...
            # NYTT: Synkronisera weather description med ACTIVE nederbördskälla
            # Om Netatmo Rain Gauge är aktiv, synkronisera med den
            # Annars synkronisera med observations om tillgänglig
            if netatmo_rain_valid and combined.get('weather_symbol'):
            ...
            elif observations_data and combined.get('weather_symbol'):
                combined['weather_description'] = self.get_observations_synchronized_description(
                    combined['weather_symbol'],
