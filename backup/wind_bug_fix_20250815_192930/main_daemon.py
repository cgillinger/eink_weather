...
🔧 MINIMAL WIND-FIX: Respekterar enabled flaggor även när module_groups finns
...
class DynamicModuleManager:
    ...
    def get_active_modules(self, context_data: Dict) -> List[str]:
        ...
        🔧 WIND-MODUL FIX: Respekterar nu enabled flaggor även när module_groups finns
        ...
        try:
            ...
            # 🔧 WIND-MODUL FIX: Lägg till enabled legacy modules även när groups finns
            enabled_legacy = [name for name, config in self.legacy_modules.items() 
                            if config.get('enabled', False) and name not in active_modules]
            if enabled_legacy:
                active_modules.extend(enabled_legacy)
                self.logger.info(f"🔧 WIND-FIX: Lade till enabled legacy moduler: {enabled_legacy}")
...
class EPaperWeatherDaemon:
    ...
    def __init__(self, config_path="config.json"):
        ...
        self.logger.info("🔧 WIND-MODUL FIX: Respekterar nu enabled flaggor även när module_groups finns")
    ...
    def draw_module_border(self, x, y, width, height, module_name):
        ...
        if module_name == 'main_weather':
            ...
            self.draw.line([(x + 2, y + height - 2), (x + width - 2, y + height - 2)], fill=0, width=1)
            self.draw.line([(x + width - 2, y + 2), (x + width - 2, y + height - 2)], fill=0, width=1)
        # 🔧 NYTT: Wind-modul border support
        elif module_name == 'wind_module':
            self.draw.rectangle([(x, y), (x + width, y + height)], outline=0, width=2)
            self.draw.rectangle([(x + 2, y + 2), (x + width - 2, y + height - 2)], outline=0, width=1)
            self.draw.line([(x + 8, y + 8), (x + 20, y + 8)], fill=0, width=1)
            self.draw.line([(x + 8, y + 8), (x + 8, y + 20)], fill=0, width=1)
