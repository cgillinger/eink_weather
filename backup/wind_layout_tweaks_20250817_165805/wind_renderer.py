...
class WindRenderer(ModuleRenderer):
    ...
    def render(self, x: int, y: int, width: int, height: int, 
        ...
        try:
            ...
            ms_bbox = self.draw.textbbox((0, 0), ms_text, font=ms_font)  # Använd textbbox istället
            ms_width = ms_bbox[2] - ms_bbox[0]
            ms_x = center_x - ms_width // 2  # Centrerad
            ms_y = y + 80  # Under ikonen
            self.draw_text_with_fallback(
                (ms_x, ms_y),
                ms_text,
                ms_font,  # Lagom font för att få plats
                fill=0
            )
            
            # 3. HUVUDDATA: BESKRIVANDE TEXT (balanserad storlek)
            desc_font = self.fonts.get('medium_main', self.fonts.get('small_main'))
            desc_bbox = self.draw.textbbox((0, 0), speed_description, font=desc_font)  # Använd textbbox istället
            desc_width = desc_bbox[2] - desc_bbox[0]
            desc_x = center_x - desc_width // 2  # Centrerad
            desc_y = ms_y + 40  # Under M/S-värdet
            self.draw_text_with_fallback(
                (desc_x, desc_y),
                speed_description,
                desc_font,  # Balanserad storlek
                fill=0
            )
            ...
            direction_font = self.fonts.get('small_main', self.fonts.get('small_desc'))
            direction_text_bbox = self.draw.textbbox((0, 0), direction_short, font=direction_font)  # Använd textbbox istället
            direction_text_width = direction_text_bbox[2] - direction_text_bbox[0]
            combined_width = 24 + 8 + direction_text_width  # ikon + mellanrum + text
            ...
            self.draw_text_with_fallback(
                ...
                direction_short,
                direction_font,  # Diskret storlek
                fill=0
            )
